...
\begin{table}[h!]
\centering
\caption{IDEA / DES（C++ 与 Python）统一口径性能对比}
\begin{tabular}{lrrr}
\hline
\textbf{指标} & \textbf{IDEA (C++)} & \textbf{DES (C++)} & \textbf{DES (Python)} \\
\hline
输入字节数 (bytes) & 2,617,258 & 2,617,258 & 2,611,018 \\
输出字节数 (bytes) & 2,617,264 & 2,617,264 & 2,611,024 \\
数据块数量 (blocks) & 327,158 & 327,158 & 326,378 \\
加密耗时 (ms) & 67.727 & 973.362 & \textbf{24.864} \\
解密耗时 (ms) & 67.925 & 975.170 & \textbf{20.087} \\
总耗时 (ms) & 139.129 & 1,949.650 & \textbf{44.974} \\
起始内存占用 (KB) & \textbf{3,504} & 3,508 & 18,220 \\
结束内存占用 (KB) & \textbf{13,180} & 11,324 & 28,668 \\
峰值内存占用 (KB) & \textbf{13,180} & 11,324 & 28,156 \\
CPU 使用率 (\%) & 5.8 & 6.2 & 5.6 \\
