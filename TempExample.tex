...
为保证可复现性，\emph{所有实现在加密模式上统一为 ECB}（仅用于可控基准测试，不代表生产安全实践）。数据来源可复核：主要语料《石头记》与对照语料《西游记》分别见[项目首页](https://github.com/Kirawii/TGP2)。
...
\subsection{运行方式与命令范式}
\paragraph{C++/IDEA：}可执行程序从标准输入读取 16~字节密钥的\emph{一行}，正文通过标准输入或``\texttt{@/path/to/file}''传入：
\begin{verbatim}
g++ IDEA.cpp -O3 -std=c++17 -o idea
echo "your_16B_key____" | ./idea < @/path/to/The_Story_of_the_Stone.txt
\end{verbatim}

\paragraph{C++/DES：}密钥读取与数据输入方式与上同，密钥长度为 8~字节：
\begin{verbatim}
g++ DES_faster.cpp -O3 -std=c++17 -o des
echo "your8key" | ./des < @/path/to/The_Story_of_the_Stone.txt
\end{verbatim}

\paragraph{Python/DES：}脚本内置 ECB+PKCS\#7 流程与统一指标打印，可直接对同一文本运行：
\begin{verbatim}
...
未来研究可在以下方向进一步拓展：
\begin{itemize}
  \item 结合多线程并行与 SIMD 指令集（如 AVX2、AVX512）优化，探索分组加密在多核 CPU 上的加速潜力；
  \item 将实现移植至 GPU 或 FPGA 平台，比较不同硬件架构下的能耗与延迟差异；
  \item 引入现代加密标准（如 AES）进行三者的性能与安全性综合评估；
  \item 在更大规模文本与实时通信数据上测试算法的稳定性与能耗表现。
\end{itemize}
